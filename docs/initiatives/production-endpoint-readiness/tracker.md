...
| 2026-05-11 | Follow-up — chart range options | completed | API contract change | `/markets/:address/charts` now supports `range=7d|30d|90d|1y`; omitted range still defaults to `30d`, UI label casing is normalized, and unsupported range returns `INVALID_CHART_RANGE`. Canonical API docs updated. Architecture docs checked; no update needed. |
